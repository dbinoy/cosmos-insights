...
def register_training_engaged_members_callbacks(app):
    ...
    @monitor_performance("Member Engagement Filter Application for Members")
    ...
    @monitor_performance("Member Engagement Filter Application for Attendance")
    ...
    @monitor_performance("Top Members Calculation for Member Engagement")
    ...
    @monitor_chart_performance("Top Engaged Members Chart Creation")
    ...
    @monitor_performance("Member Engagement Insights Generation")
    def generate_member_insights(top_members, engagement_metric, top_count):
        """
        Generate automated insights for top engaged members
        """
        if top_members.empty:
            return html.Div("No insights available - insufficient member data.", 
                        style={'color': 'gray'})
        ...
        try:
            ...
            total_members = len(top_members)
            avg_sessions = top_members['TotalSessionsAttended'].mean()
            avg_attendance_rate = top_members['AttendanceRate'].mean()
            total_training_hours = top_members['training_hours'].sum()
            
            # Top performer
            top_performer = top_members.iloc[0]
            top_name = top_performer['MemberName']
            top_value = top_performer['metric_value']
            
            # Attendance rate analysis
            perfect_attendance = top_members[top_members['AttendanceRate'] == 100]
            
            # Office distribution
            office_distribution = top_members['OfficeCode'].value_counts()
            top_office = office_distribution.index[0] if len(office_distribution) > 0 else "Unknown"
            
            # Generate insights based on metric type
            if engagement_metric == 'sessions_attended':
                insights.extend([
                    f"🏆 **Top Performer**: {top_name} leads with {int(top_value)} sessions attended",
                    f"📊 **Group Average**: Top {top_count} members average {avg_sessions:.1f} sessions attended"
                ])
            elif engagement_metric == 'training_hours':
                insights.extend([
                    f"🏆 **Most Dedicated**: {top_name} invested {top_value:.1f} training hours",
                    f"⏰ **Collective Investment**: Top {top_count} members completed {total_training_hours:.0f} total hours"
                    # ✅ Removed: f"📈 **Average Commitment**: {avg_sessions:.1f} sessions per top performer"
                ])
            else:  # topics_completed
                insights.extend([
                    f"🏆 **Most Diverse**: {top_name} completed {int(top_value)} different topics",
                    f"📚 **Knowledge Seekers**: Top {top_count} members average {avg_sessions:.1f} sessions each"
                    # ✅ Removed: f"🎯 **Quality Focus**: {avg_attendance_rate:.1f}% average attendance rate"
                ])
            
            # Add office insights if we have office data
            if len(office_distribution) > 1:
                insights.append(f"🏢 **Leading Office**: {top_office} has {office_distribution.iloc[0]} top performers")
            
            # Perfect attendance insight
            if len(perfect_attendance) > 0:
                insights.append(f"🌟 **Perfect Attendance**: {len(perfect_attendance)} members with 100% attendance rate")
    ...
    def update_engaged_members_chart(query_selections, engagement_metric, top_count):
        ...
        try:
            ...
            # ✅ Generate insights
            insights = generate_member_insights(top_members, engagement_metric, top_count)

            # print(f"✅ Engaged members chart and insights created with {len(top_members)} members")
            return fig, insights  # ✅ Return both chart and insights
            
        except Exception as e:
            print(f"❌ Error creating engaged members chart: {str(e)}")
            import traceback
            traceback.print_exc()
            error_chart = create_empty_chart("Chart Error", f"Error processing data: {str(e)}")
            error_insights = html.Div("Unable to generate insights due to error.", style={'color': 'red'})
            return error_chart, error_insights  # ✅ Return both error states
